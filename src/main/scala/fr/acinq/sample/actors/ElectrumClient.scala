...
/**
  ...
  *
  */
class ElectrumClient(serverAddress: InetSocketAddress, ssl: SSL)(implicit val ec: ExecutionContext) extends Actor with ActorLogging {
  ...
  channelOpenFuture.addListeners((future: ChannelFuture) => {
    if (!future.isSuccess) {
      errorHandler(future.cause())
    } else {
      future.channel().closeFuture().addListener((future: ChannelFuture) => {
        if (!future.isSuccess) {
          errorHandler(future.cause())
        } else {
    ...
    case Right(json: JsonRPCResponse) =>
      (parseJsonResponse(version, json): @unchecked) match {
        case ServerVersionResponse(clientName, protocolVersion) =>
          ...
          log.info("waiting for tip from server={}", serverAddress)
